...
默认采用无栏杆的开放边界，让高空平台、天阶、云桥与回廊保持开阔、危险而神圣；只有用户明确要求或严格考据需要时才加入时代可信的栏杆。画面以高明度中性色为基底，保留克制的朱砂、石青、石绿、深木或暖金材质色，避免脏灰雾罩，也避免满屏红金和霓虹艳色。
...
- 设计无栏杆开放边界、可验证巨物尺度和明亮但克制的矿物色彩。
- 区分单体天宫地标与有人居住的天界聚落，并匹配机位、距离、倾角和焦段。
...
- 结果图 URL 信任假设：下载请求不携带任何凭证，且重定向后会复核最终地址仍为 HTTPS。指向非官方兼容服务（`GPTX_BASE_URL` 覆盖）时，视为用户已信任该服务返回的下载地址。
